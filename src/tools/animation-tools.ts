...
export function registerAnimationTools(
  ...
  server.tool(
    ...
    {
      path: z.string().describe("Path to the AnimationPlayer node"),
    },
    async ({ path }) => {
      try {
  ...
  server.tool(
    ...
    async ({ path, animation_name }) => {
      try {
        ...
        }>("animation.get", { path, animation_name });
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      } catch (e) {
        return { content: [{ type: "text", text: `Error: ${(e as Error).message}` }], isError: true };
      }
  ...
  server.tool(
    ...
    async ({ path, animation_name, length, loop_mode, tracks }) => {
      try {
        ...
        const result = await bridge.send<{ success: true; animation_name: string; track_count: number }>(
          "animation.create",
          { path, animation_name, length, loop_mode: loop_mode ?? 0, tracks: parsedTracks }
        );
        return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
      } catch (e) {
        return { content: [{ type: "text", text: `Error: ${(e as Error).message}` }], isError: true };
